...
**Trạng thái**: ✅ Hoàn thành (2026-03-14)

### Checklist

- [x] Chạy `npm run generate:api` → `src/lib/api/generated/` được tạo (27 files)
  - Fix YAML: `summary` có `{}` cần được quote trong openapi.yaml
- [x] Cập nhật `client.ts`: cấu hình `OpenAPI.BASE` + `OpenAPI.TOKEN` từ localStorage
- [x] `trace/[publicCode]/page.tsx`: wire với `TraceService.getApiTrace()` + `ApiError` + `CancelablePromise.cancel()`
- [x] `types.ts`: aligned with generated (strict interfaces + EventIndexer extensions)
- [x] `tsconfig.json`: thêm `target: ES2017` cho CancelablePromise
- [x] TypeScript: 0 errors | Tests: 32/32 passed
- [x] Schema mismatch: không có — types đã align chính xác từ Tuần 2
