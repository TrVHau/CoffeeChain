...
export function QrScanner({ onCodeDetected, autoNavigate = true }: QrScannerProps) {
  ...
  useEffect(() => {
    ...
    reader
      ...
      .catch((err: unknown) => {
        setState('error');
        setErrorMsg(err instanceof Error ? err.message : 'Không thể truy cập camera.');
      });
  ...
  return (
    <div className="space-y-4">
      <div className="relative overflow-hidden rounded-xl border-2 border-rose-300 bg-black">
        <video
          ref={videoRef}
          className="w-full"
          playsInline
        ...
        {state === 'scanning' && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div className="h-40 w-40 rounded-lg border-4 border-rose-400 opacity-70" />
          </div>
        )}
        {state === 'success' && (
      ...
      {state === 'error' && (
        <p className="text-sm text-orange-600">
          {errorMsg || 'Không thể khởi động camera.'}{' '}
          <button onClick={() => window.location.reload()} className="underline hover:text-orange-800">
            Thử lại
          </button>
        </p>
      )}
      ...
      <div>
        <p className="mb-1 text-xs text-slate-500">Hoặc nhập mã sản phẩm thủ công:</p>
