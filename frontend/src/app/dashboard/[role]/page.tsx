...
export default function RoleDashboardPage({
  ...
  const roleFromPath = params.role.toUpperCase();

  return (
    <div className="min-h-screen bg-amber-50">
      <header className="flex items-center justify-between border-b border-amber-200 bg-white px-6 py-3 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="rounded-lg border border-gray-300 px-3 py-1.5 text-xs text-gray-600 transition hover:bg-gray-50"
          >
            Đăng xuất
          </button>
        </div>
      ...
      <main className="mx-auto max-w-2xl px-4 py-10">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Xin chào <span className="font-semibold text-amber-700">{user.userId}</span>. Đây là giao diện demo cho vai
            trò {roleLabel} ({roleFromPath}).
          </p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-2 gap-3">
          <a
            href="/demo"
            className="rounded-xl border border-amber-200 bg-white p-4 shadow-sm transition hover:bg-amber-50"
          >
        ...
        <div className="rounded-2xl bg-white p-6 shadow">
          ...
          <p className="mb-4 text-sm text-gray-500">Quét mã QR trên bao bì để xem hành trình sản xuất của lô hàng.</p>
          <QrScanner />
        </div>
      </main>
    </div>
